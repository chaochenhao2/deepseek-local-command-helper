...
6. **正文解析（`<command>` 提取）**：`content.js` 用 MutationObserver 监听新增的 `ds-message` 回复容器，排除思考容器后取正文，提取 `<command>...</command>` 填入输入框。思考容器按「内容特征」（textContent 以「已思考/深度思考」开头）定位，不依赖混淆 class——DeepSeek 改版时优先检查 `findMessageContainer` / `findThinkingContainer` 是否仍命中。
   ...
   - **再坑**：AI 会在正文里“提及 `<command>` 标签”一词（被转义成文本），与真正的 `<command>cmd</command>` 混在同一 textContent，导致正则从「提及处」一路吞到很远才出现的 `</command>`。修复：通道A 正则用 `[^<]`（命令内容不含 `<`，遇下一个标签即停，避免跨标签吞段），**区分代码块内外**提取——
     - **通道0（首选）**：从 `<pre>/<code>` 代码块内提取，**不过滤中文**（代码块里的一定是真实命令，含 `python -c "中文"` 这种长命令）；
     - **通道A**：代码块外普通段落提取，才用「长度>60 且含中文」过滤（避免“提及 <command> 标签”的解释文字被误提取）。
     - **五坑（重要）**：曾统一用「长且含中文丢弃」，会**误伤代码块里的含中文长命令**（如 python -c 里带大段中文文本），导致长命令不解析。必须按“是否在代码块内”区分是否过滤中文。MAX=20000。
