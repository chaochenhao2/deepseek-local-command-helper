...
6. **正文解析（`<command>` 提取）**：`content.js` 用 MutationObserver 监听新增的 `ds-message` 回复容器，排除思考容器后取正文，提取 `<command>...</command>` 填入输入框。思考容器按「内容特征」（textContent 以「已思考/深度思考」开头）定位，不依赖混淆 class——DeepSeek 改版时优先检查 `findMessageContainer` / `findThinkingContainer` 是否仍命中。
   ...
   - **再坑**：AI 会在正文里“提及 `<command>` 标签”一词（被转义成文本），与真正的 `<command>cmd</command>` 混在同一 textContent，导致正则从「提及处」一路吞到很远才出现的 `</command>`。修复：通道A 正则用 `[^<]{1,4000}`（命令内容不含 `<`，遇下一个标签即停，避免跨标签吞段），并对「长度>120 且含中文」的内容判为解释文字丢弃——既容纳长代码型命令（`python -c "..."`），又排除中文解释误匹配。**注意长度上限 4000**：长命令若超 4000 字符仍会匹配不到，但实践中极少；若再遇长命令不解析，先查此上限。
