...
6. **正文解析（`<command>` 提取）**：`content.js` 用 MutationObserver 监听新增的 `ds-message` 回复容器，排除思考容器后取正文，提取 `<command>...</command>` 填入输入框。思考容器按「内容特征」（textContent 以「已思考/深度思考」开头）定位，不依赖混淆 class——DeepSeek 改版时优先检查 `findMessageContainer` / `findThinkingContainer` 是否仍命中。
   ...
   - **生成锁（最可靠）**：`isDeepSeekGenerating()` 通过发送按钮 SVG path 判断是否在生成——空闲=向上箭头(`M8.3125 0.98`)，生成中=圆角方块停止(`M2 4.88C2 3.68`)。`scanMessage` 开头若检测到正在生成则**拒绝执行**并 `scheduleScan` 延迟再扫，从根本上避免抓不完整命令。注意：发送按钮 class 无法区分（生成中/空闲都可能 `dis=false`），只能看 SVG path；DeepSeek 改版若换图标需更新此判断。
