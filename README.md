...
面板里的「自动发送延迟(秒)」输入框控制全自动模式：

- **留空**：关闭自动（默认）。AI 命令自动填入输入框，但需你手动点「发送」执行；结果也只填入 DeepSeek 输入框，不自动发送。
- **填数字**（如 `2`）：开启全自动——
  1. AI 回复里解析到 `<command>` 后，**自动填入并立即执行**该命令（无需手动点发送）；
  2. 命令执行拿到结果后，等待**延迟秒数**（`0` 为立即）；
  3. 自动把结果填入 DeepSeek 输入框，并**自动点击 DeepSeek 的发送按钮**发给它，形成完整闭环。
